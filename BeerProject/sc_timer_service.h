...
private:
	struct TimeBased {
		...
		sc_eventid pt_evid;
		sc_boolean periodic;
		TimeBased() : handle(sc_null), pt_evid(0), periodic(false) {}
	};
	struct NonEmpty {
		sc_time time_ms;
		TimeBased time_event;
		CycleBasedInterface* run_cycle_handle;
		NonEmpty() : time_ms(0), time_event(), run_cycle_handle(sc_null) {}
	};
public:
	struct TaskData {
		enum TaskType {
		...
		TaskData(TimedInterface* time_event_handle, sc_eventid pt_evid, sc_time time_ms, sc_boolean periodic) : 
		...
		TaskData(CycleBasedInterface* run_cycle_handle, sc_time time_ms) : 
			type(RUNCYCLE_TASK) {
			get.time_ms = time_ms;
	...
	void updateElapsedTimeMs(sc_time elapsed_time_ms_ = 0) {
...
public:
	 ...
	 */
	virtual void setRuncycleTimerFor(CycleBasedInterface *statemachine, sc_time cycle_period);
	 ...
	 */
	virtual void proceed(sc_time elapsed_ms);
