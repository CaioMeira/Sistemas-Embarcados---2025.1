...
void TimerService::setTimer(TimedInterface *statemachine, sc_eventid event,
		sc_time time_ms, sc_boolean isPeriodic) {
 ...
 */
void TimerService::setRuncycleTimerFor(CycleBasedInterface *statemachine,  sc_time cycle_period) {
...
*/
void TimerService::proceed(sc_time elapsed_ms) {
	if (next_active_task >= length) {
		return;
	}
	
	sc_time time_to_proceed = std::min(time_till_next_task(), elapsed_ms);
	sc_time proceeded_time = 0;
	while (time_to_proceed > 0) {
		...
		do {
			...
			sc_time best_remaining_time = tasks[best].data.get.time_ms - tasks[best].elapsed_time_ms;
			...
			while (next_task < length) {
				...
				sc_time remaining_time = current_task.data.get.time_ms - current_task.elapsed_time_ms; 
...
sc_time TimerService::time_till_next_task() {
	if (next_active_task == length) {
		return 0;
	}
	
	sc_time time = tasks[next_active_task].data.get.time_ms - tasks[next_active_task].elapsed_time_ms;
